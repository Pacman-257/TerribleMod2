...
public class NapoleonEntity extends Monster {
	private final ServerBossEvent bossInfo = new ServerBossEvent(this.getDisplayName(), ServerBossEvent.BossBarColor.BLUE, ServerBossEvent.BossBarOverlay.PROGRESS);

	public NapoleonEntity(EntityType<NapoleonEntity> type, Level world) {
		super(type, world);
		xpReward = 50;
		setNoAi(false);
		setCustomName(Component.literal("Napoleon"));
	...
	public boolean hurt(DamageSource damagesource, float amount) {
		...
		NapoleonEntityIsHurtProcedure.execute(world, x, y, z);
	...
	public void die(DamageSource source) {
		super.die(source);
		NapoleonEntityDiesProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}

	@Override
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata) {
		SpawnGroupData retval = super.finalizeSpawn(world, difficulty, reason, livingdata);
		NapoleonOnInitialEntitySpawnProcedure.execute(world, this.getX(), this.getY(), this.getZ());
		return retval;
	}
