...
public class MarketPlierEntity extends Monster {
	private final ServerBossEvent bossInfo = new ServerBossEvent(this.getDisplayName(), ServerBossEvent.BossBarColor.RED, ServerBossEvent.BossBarOverlay.PROGRESS);

	public MarketPlierEntity(EntityType<MarketPlierEntity> type, Level world) {
		super(type, world);
		xpReward = 50;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(5, new NearestAttackableTargetGoal(this, MarkiplierEntity.class, false, false));
	...
	public void die(DamageSource source) {
		super.die(source);
		MarketPlierEntityDiesProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	}

	@Override
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata) {
		SpawnGroupData retval = super.finalizeSpawn(world, difficulty, reason, livingdata);
		MarketPlierOnInitialEntitySpawnProcedure.execute(world, this.getX(), this.getY(), this.getZ());
		return retval;
	}
